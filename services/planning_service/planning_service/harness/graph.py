...
def _needs_approval(deps: RunDeps, name: str) -> bool:
    descriptor = deps.registry.by_name().get(name)
    return bool(descriptor is not None and descriptor.policy.requires_approval)
...
def build_graph(deps: RunDeps):
    ...
    def tools_node(state: HarnessState) -> dict[str, Any]:
        ...
        calls = _tool_calls_from_message(state["messages"][-1])
        pending = [c for c in calls if _needs_approval(deps, str(c["name"]))]
        ...
        ok = any(
            ...
            and r["result"].get("status") in ("ok", "proposed")
            for r in records
        )
